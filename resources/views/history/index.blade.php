<x-app-layout>
        ...
        $safeDate = function($v, $fmt = 'd-m-Y') use ($tz) {
            try {
                ...
                if ($v instanceof \DateTimeInterface) {
                    return \Carbon\Carbon::instance($v)->tz($tz)->format($fmt);
                }
                if (is_numeric($v)) {
                    $n = (float) $v;
                    if ($n > 25569 && $n < 600000) {
                        $ts = ($n - 25569) * 86400;
                        return \Carbon\Carbon::createFromTimestampUTC((int) round($ts))->tz($tz)->format($fmt);
                    }
                    if ($n > 1e9 && $n < 4102444800) {
                        return \Carbon\Carbon::createFromTimestampUTC((int) $n)->tz($tz)->format($fmt);
                    }
                }
                $s = trim((string) $v);
                if (preg_match('/\d/', $s)) {
                    return \Carbon\Carbon::parse($s)->tz($tz)->format($fmt);
                }
                return '-';
            } catch (\Throwable $e) {
                return '-';
    ...
    <div class="wrap">
        <div class="card">
            ...
            {{-- ✅ Import Button --}}
            <form action="{{ route('history.import.google') }}" method="POST">@csrf
            ...
            <form method="GET" action="{{ route('history.index') }}" style="margin-bottom:12px;">
                <div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;">
                    <input type="text" name="user" placeholder="Search UserID or Name"
                        value="{{ request('user') }}"
                    ...
                    <tbody>
                        @foreach($history as $row)
                            @php
                                ...
                                $borrowedAtRaw = $row['BorrowedAt'] ?? $row['BorrowTime'] ?? $row['BorrowedTime'] ?? null;
    ...
    <script>
        function updateDays() {
            ...
            badges.forEach(el => {
                ...
                let diff = Math.ceil((ed - sd) / 86400000);
                if (diff < 1) diff = 1;
